#' FEWS Multiplant plants -- lake, pond, river (version 3.104)
#'
...
#'          (.xlsx, .csv, or both)
#'
#' @return Microsoft Excel spreadsheet with the output parameters and a plot of
#'           the average evaporation per month
#'
...
#'    \item r - How to replace NA values in a table *for selected columns*? data.frame, data.table - - Stack Overflow answered by eddi on Oct 15 2013. See \url{http://stackoverflow.com/questions/19379081/how-to-replace-na-values-in-a-table-for-selected-columns-data-frame-data-tab}.
...
if (grepl("*.xls", file)) { # Source 20

ifelse (class(sheet) == "numeric", sheetname <- excel_sheets(file)[sheet], sheetname <- sheet) # obtain the sheet name

fewsronly <- read_excel(file, sheet = sheet)
fewsronly <- data.frame(fewsronly)
fewsronly <- setDT(fewsronly)

} else {

fewsronly <- fread(file)


## Input values
minheatload <- as.numeric(fewsronly[3, 1, with = FALSE][[1]])
...
inputwind <- fewsronly[3, 2:4, with = FALSE] # list wind1, wind2, and wind3 as a data.table
...
cols <- names(fewsronly[, NAduty, with = FALSE]) # create cols, which is the names of those column numbers from NAduty
...
pond <- fewsronly[, c(1, 5), with = FALSE] # subset of fewsronly with the Plant ID and Pond Area (acres) data only

duty <- fewsronly[, c(1, 6:17), with = FALSE] # subset of fewsronly with the Plant ID and Duty [Added heat load (MMBtu)] data only

db <- fewsronly[, c(1, 18:29), with = FALSE] # subset of fewsronly with the Plant ID and Dry bulb air temperature Ta (deg C) data only

wb <- fewsronly[, c(1, 30:41), with = FALSE] # subset of fewsronly with the Plant ID and Wet bulb air temperature Twb (deg C) data only
...
wt <- fewsronly[, c(1, 42:53), with = FALSE] # subset of fewsronly with the Plant ID and Natural water temperature T (deg C) data only

ws <- fewsronly[, c(1, 54:65), with = FALSE] # subset of fewsronly with the Plant ID and Wind speed at 2m W (mph) data only
...
pond[, paste0("Pond Area (acres)", 2:12) := rep(pond[, 2, with = FALSE], 11)]
addedheatload <- data.table(duty, pond, monthdays)
addedheatload[, 14 := NULL, with = FALSE]
...
# air vapor pressure
fewsronly2 <- fewsronly[, c(1, 4), with = FALSE]
fewsronly2[, paste0("Pressure, mbar", 2:12) := rep(fewsronly2[, 2, with = FALSE], 11)]
ea <- data.table(e_Twb, db, wb, fewsronly2)
ea[, c(14, 27, 40) := NULL, with = FALSE]
...
calc_unheated_condition[, c(14, 27, 40, 53, 66, 79, 92) := NULL, with = FALSE]
...
deltaT_heat_loading_ratio_initial[, c(14, 27) := NULL, with = FALSE]
...
deltaE1[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC1[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH1[, c(14, 27) := NULL, with = FALSE]
...
percentchangeTprime1[, 14 := NULL, with = FALSE]
...
deltaE2[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC2[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH2[, c(14, 27) := NULL, with = FALSE]
...
iteration2[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]


## iteration 3
# Estimated heated water temperature T' (deg C)1
T3 <- data.table(T1, T2, deltaH1, deltaH2, deltaHf)
T3[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
percentchangeTprime2[, 14 := NULL, with = FALSE]
...
deltaE3[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC3[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH3[, c(14, 27) := NULL, with = FALSE]
...
iteration3[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]
...
T4[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
percentchangeTprime3[, 14 := NULL, with = FALSE]
...
deltaE4[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC4[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH4[, c(14, 27) := NULL, with = FALSE]
...
iteration4[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]


## iteration 5
# Estimated heated water temperature T' (deg C)1
T5 <- data.table(T3, T4, deltaH3, deltaH4, deltaHf)
T5[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
percentchangeTprime4[, 14 := NULL, with = FALSE]
...
deltaE5[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC5[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH5[, c(14, 27) := NULL, with = FALSE]
...
iteration5[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]
...
T6[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
percentchangeTprime5[, 14 := NULL, with = FALSE]
...
deltaE6[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC6[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH6[, c(14, 27) := NULL, with = FALSE]
...
iteration6[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]


## iteration 7
# Estimated heated water temperature T' (deg C)1
T7 <- data.table(T5, T6, deltaH5, deltaH6, deltaHf)
T7[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
percentchangeTprime6[, 14 := NULL, with = FALSE]
...
deltaE7[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC7[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH7[, c(14, 27) := NULL, with = FALSE]
...
iteration7[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]
...
T8[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
percentchangeTprime7[, 14 := NULL, with = FALSE]
...
deltaE8[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC8[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH8[, c(14, 27) := NULL, with = FALSE]
...
iteration8[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]
...
T9[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
percentchangeTprime8[, 14 := NULL, with = FALSE]
...
deltaE9[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaC9[, c(14, 27, 40, 53) := NULL, with = FALSE]
...
deltaH9[, c(14, 27) := NULL, with = FALSE]
...
iteration9[, c(14, 27, 40, 53, 66, 79) := NULL, with = FALSE]
...
heat_vaporization[, 14 := NULL, with = FALSE]
...
percent_forced_evap <- data.table(iteration9[, 1, with = FALSE], iteration9[, c(38:49), with = FALSE], iteration9[, c(74:length(iteration9)), with = FALSE])
...
cond_duty_per_evap_vol[, 14 := NULL, with = FALSE]
...
## Final groups
# output
outputfinal <- iteration9[, c(1:13), with = FALSE]


# Output values for evaporation depth
output_evap_depth <- data.table(final_rho, heat_vapor, iteration9[, c(26:37), with = FALSE])
output_evap_depth[, 14 := NULL, with = FALSE]


# Output
# Increment in heat loss by mode
output_incr_heat_loss_mode <- iteration9[, c(1, 38:73), with = FALSE]
...
heat_water_temp_Tprime <- iteration8[, c(1:13), with = FALSE]
...
deltaT_heat_loading_ratio_final[, c(14, 27) := NULL, with = FALSE]
...
percent_change_Tprime_final_iteration <- data.table(iteration9[, c(1:13), with = FALSE], iteration8[, c(1:13), with = FALSE])
percent_change_Tprime_final_iteration[, 14 := NULL, with = FALSE]
...
heat_water <- data.table(iteration9[, c(1:13), with = FALSE], wt)
heat_water[, 14 := NULL, with = FALSE]
...
add_evap <- data.table(iteration9[, c(1, 38:49), with = FALSE], heat_vaporization, monthdays)
add_evap[, 14 := NULL, with = FALSE]
...
# iteration9[, c(1, 38:49), with = FALSE]


# Total heat loss increment = final deltaH
# iteration9[, c(74:length(iteration9)), with = FALSE]
...
evap_cool_percent_add_heat <- data.table(addedheatload, iteration9[, c(38:49), with = FALSE], iteration9[, c(74:length(iteration9)), with = FALSE])
...
evap_cool_MGD_month[, c(14, 27) := NULL, with = FALSE]
...
sum_mean_MGD_year_consumpt <- sum(mean_MGD_year_consumpt[, 14, with = FALSE], na.rm = TRUE)
...
est_min_MGD_withdrawal[, 14 := NULL, with = FALSE]
...
# Estimated Minimum
mean_MGD_year1_withdr <- sum(mean_MGD_year_withdr[, 14, with = FALSE], na.rm = TRUE)
...
est_MGD_withdrawal[, 14 := NULL, with = FALSE]
...
mean_MGD_year1_withdr2 <- sum(mean_MGD_year_withdr2[, 14, with = FALSE], na.rm = TRUE)
...
est_max_MGD_withdrawal[, 14 := NULL, with = FALSE]
...
# Estimated Withdrawal
mean_MGD_year1_withdr3 <- sum(mean_MGD_year_withdr3[, 14, with = FALSE], na.rm = TRUE)
...
Total_duty_year_ID <- Total_duty_year_ID[, c(1, 14), with = FALSE]

Total_duty_year_ID_check <- sum(Total_duty_year_ID[, 2, with = FALSE])


# Evaporation energy - product of FEpercent and condenser duty
evap_energy <- data.table(duty, evap_cool_percent_add_heat)
evap_energy[, 14 := NULL, with = FALSE]
...
evap_energy_year_ID <- evap_energy_year_ID[, c(1, 14), with = FALSE]


evap_energy_year <- sum(evap_energy_year_ID[, 2, with = FALSE])


# percent evaporation
pct_evap <- data.table(evap_energy_year_ID, Total_duty_year_ID)
pct_evap[, 3 := NULL, with = FALSE]
...
write.csv(csvout, file = filesave1, row.names = FALSE, na = "")
